...
pull()
{
    git clone git@github.com:jacokok/nix.git /tmp/nix
}

disko()
{
    device=${1:-/dev/nvme0n1}
    nix --experimental-features "nix-command flakes" run github:nix-community/disko -- --mode disko /tmp/nix/hosts/disko.nix --arg device '"$device"'
}
